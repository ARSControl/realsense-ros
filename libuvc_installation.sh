...
wget https://github.com/IntelRealSense/librealsense/archive/refs/tags/v2.39.0.zip
cd /home/jetson/Downloads
unzip ./librealsense-2.39.0.zip -d .
# cd ./librealsense-master
cd /home/jetson/Downloads/librealsense-2.39.0
echo Install udev-rules
sudo cp config/99-realsense-libusb.rules /etc/udev/rules.d/ 
sudo cp config/99-realsense-d4xx-mipi-dfu.rules /etc/udev/rules.d/
sudo udevadm control --reload-rules && sudo udevadm trigger 
mkdir build && cd build
cmake ../ -DFORCE_LIBUVC=true -DCMAKE_BUILD_TYPE=release -DBUILD_WITH_CUDA=true -DCMAKE_CUDA_ARCHITECTURES=native -DCMAKE_CUDA_COMPILER=/usr/local/cuda-10.2/bin/nvcc -DFORCE_RSUSB_BACKEND=TRUE
make -j2
sudo make install
